...
from triadllm.tools.firecrawl_handlers import FirecrawlHandlersMixin
...
class ToolBroker(FirecrawlHandlersMixin):
    def __init__(
        self,
    ...
    async def execute(
        ...
        if permission_mode == PermissionMode.ASK and (approval_handler is None or not await approval_handler(request)):
            return ToolResult(
                tool=request.tool,
                success=False,
